    ...
    <div class="relative w-full h-full flex flex-col items-center justify-center p-4 gap-10">
        <div>
            <div class="flex flex-col gap-1 items-center">
                <div class="px-4 text-4xl garnet">EDIT TASK</div>
        ...
        <div class="flex flex-col items-center justify-center gap-4 w-full">
            ...
            @endif
            <form action="{{ route('setting.task.update', $target->id) }}" method="POST" class="flex flex-col gap-4 w-full max-w-lg">
                @csrf
                @method('PATCH')
                <div class="flex flex-col gap-2">
                    ...
                    <select name="rank_id" id="rank_id" class="text-gray-600 rounded-lg ml-4">
                        ...
                        @foreach($ranks as $rank)
                            <option value="{{ $rank->id }}" @if($rank->id === $target->rank_id) selected @endif>{{ $rank->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex flex-col gap-2">
                    ...
                    <select name="category_id" id="category_id" class="text-gray-600 rounded-lg ml-4">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" @if($category->id === $target->category_id) selected @endif>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex flex-col gap-2">
                    ...
                    <select name="group_id" id="group_id" class="text-gray-600 rounded-lg ml-4">
                        ...
                        @foreach($groups as $group)
                            <option value="{{ $group->id }}" @if($group->id === $target->group_id) selected @endif>{{ $group->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex flex-col gap-2">
                    ...
                    <select name="item_id" id="item_id" class="text-gray-600 rounded-lg ml-4">
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id }}" @if($item->id === $target->item_id) selected @endif>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex flex-col gap-2">
                    <label for="text" class="uppercase text-xl eng-italic">group</label>
                    <input type="text" id="text" name="text" class="rounded-lg ml-4 text-gray-600" value="{{ $target->text }}">
                </div>
